...
const registerUser = async (req, res) => {
  try {
    ...
    // Further logic to register the user goes here
  } catch (error) {
...
const loginUser = async (req, res) => {
  try {
    ...
    if (isMatch) {
      const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET);
      res.json({ success: true, token }); //kya token baar baar change hoga
    } else {
...
const updateProfile = async (req, res) => {
  try {
    ...
    // Safely parse address if provided
    if (address) {
      try {
    ...
    // If image is present, upload and add URL
    if (imageFile) {
      const imageUpload = await cloudinary.uploader.upload(imageFile.path, {
        resource_type: "image",
      });
...
const bookAppointment = async (req, res) => {
  try {
    const { userId } = req.user; //why doing in this way not through req.body
